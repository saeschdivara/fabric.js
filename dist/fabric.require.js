...
(function() {
    ...
    function getXYCoords(rotateX, rx, ry, ox, oy, x, y) {
        ...
        rx = Math.abs(rx);
        ry = Math.abs(ry);
        var px = cosTh * (ox - x) + sinTh * (oy - y), py = cosTh * (oy - y) - sinTh * (ox - x), pl = px * px / (rx * rx) + py * py / (ry * ry);
        pl *= .25;
        if (pl > 1) {
            pl = Math.sqrt(pl);
            rx *= pl;
    ...
    function segmentToBezier(cx, cy, th0, th1, rx, ry, sinTh, cosTh) {
        ...
        var sinTh0 = Math.sin(th0), cosTh0 = Math.cos(th0), sinTh1 = Math.sin(th1), cosTh1 = Math.cos(th1), a00 = cosTh * rx, a01 = -sinTh * ry, a10 = sinTh * rx, a11 = cosTh * ry, thHalf = .25 * (th1 - th0), t = 8 / 3 * Math.sin(thHalf) * Math.sin(thHalf) / Math.sin(thHalf * 2), x1 = cx + cosTh0 - t * sinTh0, y1 = cy + sinTh0 + t * cosTh0, x3 = cx + cosTh1, y3 = cy + sinTh1, x2 = x3 + t * sinTh1, y2 = y3 - t * cosTh1;
...
(function(global) {
        ...
        for (var rule in fabric.cssRules) {
            ...
            if (ruleMatchesElement) {
                for (var property in fabric.cssRules[rule]) {
                    var attr = normalizeAttr(property), value = normalizeValue(attr, fabric.cssRules[rule][property]);
                    styles[attr] = value;
                }
            }
        }
    ...
    fabric.parseSVGDocument = function() {
        ...
        return function(doc, callback, reviver) {
            ...
            var viewBoxAttr = doc.getAttribute("viewBox"), widthAttr = parseFloat(doc.getAttribute("width")), heightAttr = parseFloat(doc.getAttribute("height")), viewBoxWidth, viewBoxHeight;
...
(function() {
    ...
    fabric.util.object.extend(fabric.Object.prototype, {
        ...
        getCurrentTransformation: function() {
            var vpt = this.getViewportTransform(), tm = this.transformMatrix;
            if (vpt[0] == 1 && vpt[1] == 0 && vpt[2] == 0 && vpt[3] == 1 && vpt[4] == 0 && vpt[5] == 0 && tm != null) {
                return tm;
            } else {
                return vpt;
            }
        ...
        setCoords: function() {
            var strokeWidth = this.strokeWidth > 1 ? this.strokeWidth : 0, theta = degreesToRadians(this.angle), w = this.width, h = this.height, capped = this.strokeLineCap === "round" || this.strokeLineCap === "square", vLine = this.type === "line" && this.width === 1, hLine = this.type === "line" && this.height === 1, strokeW = capped && hLine || this.type !== "line", strokeH = capped && vLine || this.type !== "line", transformation = this.getCurrentTransformation(), f = function(p) {
...
fabric.util.object.extend(fabric.Object.prototype, {
    ...
    getSvgTransform: function() {
        var toFixed = fabric.util.toFixed, angle = this.getAngle(), vpt = this.getViewportTransform(), center = fabric.util.transformPoint(this.getCenterPoint(), vpt), NUM_FRACTION_DIGITS = fabric.Object.NUM_FRACTION_DIGITS, translatePart = "translate(" + toFixed(center.x, NUM_FRACTION_DIGITS) + " " + toFixed(center.y, NUM_FRACTION_DIGITS) + ")", anglePart = angle !== 0 ? " rotate(" + toFixed(angle, NUM_FRACTION_DIGITS) + ")" : "", scalePart = this.scaleX === 1 && this.scaleY === 1 && vpt[0] === 1 && vpt[3] === 1 ? "" : " scale(" + toFixed(this.scaleX * vpt[0], NUM_FRACTION_DIGITS) + " " + toFixed(this.scaleY * vpt[3], NUM_FRACTION_DIGITS) + ")", flipXPart = this.flipX ? "matrix(-1 0 0 1 0 0) " : "", flipYPart = this.flipY ? "matrix(1 0 0 -1 0 0)" : "";
...
(function() {
    ...
    fabric.util.object.extend(fabric.Object.prototype, {
        ...
        drawBorders: function(ctx) {
            ...
            var padding = this.padding, padding2 = padding * 2, vpt = this.getCurrentTransformation();
            ctx.save();
            ...
            if (this.hasRotatingPoint && this.isControlVisible("mtr") && !this.get("lockRotation") && this.hasControls) {
                var rotateHeight = (this.flipY ? height + padding * 2 : -height - padding * 2) / 2;
                ctx.beginPath();
        ...
        drawControls: function(ctx) {
            ...
            var size = this.cornerSize, size2 = size / 2, vpt = this.getCurrentTransformation(), strokeWidth = this.strokeWidth > 1 ? this.strokeWidth : 0, w = this.width, h = this.height, capped = this.strokeLineCap === "round" || this.strokeLineCap === "square", vLine = this.type === "line" && this.width === 1, hLine = this.type === "line" && this.height === 1, strokeW = capped && hLine || this.type !== "line", strokeH = capped && vLine || this.type !== "line";
            if (vLine) {
                w = strokeWidth;
            } else if (hLine) {
                h = strokeWidth;
            ...
            if (this.hasRotatingPoint) {
                this._drawControl("mtr", ctx, methodName, left + width / 2 - scaleOffset, this.flipY ? top + height + this.rotatingPointOffset - this.cornerSize / 2 + padding : top - this.rotatingPointOffset - this.cornerSize / 2 - padding);
            }
            ctx.restore();
            return this;
        },
...
(function(global) {
    ...
    fabric.Rect = fabric.util.createClass(fabric.Object, {
        ...
        _render: function(ctx) {
            ...
            var rx = this.rx ? Math.min(this.rx, this.width / 2) : 0, ry = this.ry ? Math.min(this.ry, this.height / 2) : 0, w = this.width, h = this.height, x = -w / 2, y = -h / 2, isInPathGroup = this.group && this.group.type === "path-group", isRounded = rx !== 0 || ry !== 0, k = 1 - .5522847498;
            ctx.beginPath();
